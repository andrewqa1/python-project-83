...
class UrlDbService:
    ...
    def create_url(self, url: str) -> int | None:
        ...
        return [
            {
                "id": url.id,
                "name": url.name,
                "created_aty": url.created_at
            } for url in returned_urls
        ]

    def get_url(self, ind: int) -> dict | None:
        ...
        } if returned_url else None

    def list_urls_with_checks(self) -> list[dict]:
        query = ("SELECT u.id, u.name, json_agg(json_build_object("
...
class UrlCheckDbService:
    ...
    def create_url_check(self, url_id: int, url_check: dict) -> int | None:
        if self.validator.validate(url_check):
            ...
            try:
                ...
                raise InvalidUrlCheckException(detail=f"Error inserting url check!") from exc
            else:
                return returned.id
        else:
            raise InvalidUrlCheckException(detail=f"Invalid url check {url_check} format!")

    def get_url_checks(self, url_id: int) -> list[dict] | None:
        query = f"SELECT id, status_code, h1, title, description, created_at FROM url_checks WHERE url_id = {url_id}"
